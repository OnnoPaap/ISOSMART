...
if (strpos($querystring,"query=")!==false) {
	...
	if (strpos($querystring,"default-graph-uri=")!==false) {
		...
		if ($shortNameSpace == 'all') {
			...
			$s=$sparqlUri."?default-graph-uri=http://data.15926.org/rdl&default-graph-uri=http://data.15926.org/dm&default-graph-uri=http://data.15926.org/prov&default-graph-uri=http://data.15926.org/lci".'&'.$querystring; 
		} else {
			$s=$sparqlUri . '?default-graph-uri='. str_replace('https://','http://',str_replace('.xyz','.org', $currentHost)) . '/' . $shortNameSpace .'&default-graph-uri=http://data.15926.org/dm&default-graph-uri=http://data.15926.org/prov&default-graph-uri=http://data.15926.org/lci&'.$querystring; 
		}
	}	
